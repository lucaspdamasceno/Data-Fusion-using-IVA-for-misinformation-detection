function [W,cost,jointisi,avgisi,iter] = iva_m_emk(X,varargin)
...
for iter = 1:Params.maxIter
   ...
   if supplyA
      ...
      if outputISI
         jointisi(iter)=amari_joint_isi;
         avgisi(iter)=amari_avg_isi;
      end
   end
   
   W_old=W; % save current W as W_old
   
   [cost(iter),~,Lambda,~,r]=comp_mpe_cost_m_emk(W,X); % Computed by M-EMK
                                                            %     r -> Constraints
                                                            % fCst5 -> Gaussian Kernel
...
%        fprintf('\n %f \n',hnk);
       ...
       for k=1:K
           ...
           %Derivative of the Gaussian Kernel wrt ynk (5º constraint)
           Sigma = std(yn(k,:,:)');
           Mu = mean(yn(k,:,:)');
           fCst5 = (1/(Sigma*sqrt(2*pi)))*exp((-1/2*Sigma)*((yn(k,:,:)'- Mu).^2));
           fCst5diff = (-1)*fCst5.*((yn(k,:,:)'- Mu)/Sigma^2);
           %fCst5diff = -1*normpdf(yn(k,:,:)').*((yn(k,:,:)' - Mu)/Sigma^2); 
           
           % Derivative of cost function with respect to ynk
           phi = (-1)*(Lambda(1)*r(1).diff(yn(k,:,:))...
                      +Lambda(2)*r(2).diff(yn(k,:,:))...
                      +Lambda(3)*r(3).diff(yn(k,:,:))...
                      +Lambda(4)*r(4).diff(yn(k,:,:))...
                      +Lambda(5)*fCst5diff'); 
           
           %gradient update rule
           dW = (X(:,:,k)/T)*phi' - hnk(:,k)/(W(n,:,k)*hnk(:,k));
