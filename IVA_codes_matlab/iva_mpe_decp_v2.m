function [W,cost,shapePass,isi,iter] = iva_mpe_decp_v2(X,varargin)
...
% in the same score function as used in the original IVA to within a scale
% factor.  This implementation also makes use of decoupling method to
% achieve 'Newton' like convergence. (see references below) and 
...
% 
% shapePass - the shape parameter used in the decomposition. It is one of
% those listed in Params.shapeList
%
...
% [4] M. Anderson, T. Adalı, & X.-L. Li,  "Joint Blind Source Separation of Multivariate Gaussian Sources: Algorithms and Performance Analysis," IEEE Trans. Signal Process., 2012, 60, 1672-1683
...
Params=struct( ...
   'shapeList',[0.5, 1, 4], ... % list of shape parameters
   'whiten',true, ... % whitening is optional
...
gammaShape=(gamma((2+K)./(2*Params.shapeList)) / K ./ ...
   gamma(K/2./Params.shapeList)).^Params.shapeList;
...
for iter = 1:Params.maxIter
   ...
   [cost(iter),zois,shapeHat]=comp_mpe_cost(W,X,Params.shapeList);
   shapePass(:,iter) = shapeHat;
   Q=0; R=0;
   
   for n=1:N      
      
      shapeParam=shapeHat(n);
      iShape=find(Params.shapeList==shapeParam,1);
      ...
      for k=1:K
         ...
         phi=(invRyn(k,:)*yn).*gipyn.^(shapeParam-1)*2*shapeParam*gammaShape(iShape);
...
function [cost,y,shapeParam,cost_derivative]=comp_mpe_cost(W,X,shape_list,const_log,uncorr)
...
if iscell(W)
   ...
   scalesources=false;
   [N,T,K]=size(X);
   shapeParam=shape_list(1)*ones(N,1);
   Y=X;
   cost=0;
   if isempty(const_log)
      xi_shape=gamma((2+K)./(2*shape_list)) / ...
         K ./ gamma(K/2./shape_list);
      const_Ck=shape_list.*gamma(K/2).*xi_shape.^(K/2)./ ...
         (pi^(K/2)*gamma(K./(2*shape_list)));      
   ...
   for n=1:N
      ...
      if uncorr
         ...
         CovN=yn*yn'/T;
         gip=dot(yn,CovN\yn);
         dcost_all=(const_log + 0.5*log(det(CovN)));
         for ishape=1:length(shape_list)
            dcost_all(ishape)= dcost_all(ishape) + ...
              xi_shape(ishape)^shape_list(ishape)* ...
               mean(gip.^shape_list(ishape));
         end
         [dcost,ishape]=min(dcost_all);
         shapeParam(n)=shape_list(ishape);
      end
      cost=cost+dcost;
   end
   y=Y;
