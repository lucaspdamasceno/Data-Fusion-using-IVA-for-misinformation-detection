function [W,cost,isi,iter] = iva_laplace_decp(X,varargin)
% [W,cost,isi] = iva_laplace_decp(X,varargin)
% Note: the code takes both real-valued and complex-valued input data
%
...
% 3-dimensional matrix of dimensions N x T x K.  The latter enforces the
...
for iter = 1:Params.maxIter
   ...
   % Inner Product of N SCVs (used to compute score function
   sqrtYtY=sqrt(sum(abs(Y).^2,3));
   ...
   for n=1:N
      ...
      for k=1:K         
         phi=Y(n,:,k)./sqrt(sum(abs(Y(n,:,:)).^2,3));
         
         if 1==1
            % decoupling implies that scalars of the gradient term matter
            % to observe this set the above to 1==0
            dW=(K+1)/2*(X(:,:,k)*phi')/T - hnk(:,k)/(W(n,:,k)*hnk(:,k));
         else
