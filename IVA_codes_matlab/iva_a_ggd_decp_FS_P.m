function [W,cost,shapePass,isi,iter] = iva_a_ggd_decp_FS_P(X,varargin)
...
for iter = 1:Params.maxIter
   ...
   [cost(iter),~,shapeHat]=comp_mpe_cost_FS(W,X);
   shapePass(:,iter) = shapeHat;
   Q=0; R=0;
   
   parfor n=1:N
    yn(:,:,n)=squeeze(Y(n,:,:))';
    [Ryn(:,:,n), b_hat(n), m(n)] = fisher_scoring_complete(yn(:,:,n),0,100);
    invRyn(:,:,n)=inv(Ryn(:,:,n));
    gipyn(:,:,n)=dot(yn(:,:,n),invRyn(:,:,n)*yn(:,:,n));
   
   %Cost Calculation
    const_Ck=((1/2).^(K./(2.*b_hat(n)))).*(b_hat(n).*gamma(K/2)./(pi^(K/2)*gamma(K./(2*b_hat(n)))));
    const_log=-log(const_Ck);
    gip=dot(yn(:,:,n),Ryn(:,:,n)\yn(:,:,n));
    dcost_all=const_log + (K/2)*log(m(n)) + 0.5*log(det(Ryn(:,:,n))) + (mean(gip.^b_hat(n))/(2*m(n)^(b_hat(n))));
    dcost(n)=dcost_all;
   end
   cost(iter) = cost(iter) + sum(dcost);
   shapePass(:,iter) = b_hat;

   alpha_scale = Params.alpha0;
   for n=1:N
       [hnk,Q,R]=decouple_trick(W,n,Q,R);
       for k=1:K
           % Derivative of cost function with respect to wnk
           phi=(invRyn(k,:,n)*yn(:,:,n)).*gipyn(:,:,n).^(b_hat(n)-1)*b_hat(n)*(m(n)^(-b_hat(n)));
