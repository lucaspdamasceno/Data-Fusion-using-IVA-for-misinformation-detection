...
S = genMixGGDmD( paramGGD, 1, 1, K, T ); % Dataset KxT
%data = importdata('aerogerador.mat');
%S = data';
...
for iter = 2 : maxIter
    ...
    %Plot approximated pdf using all the constraints (iterative)
    %f = p(X,LambdaNew);
    %A = normalization(r,LambdaNew,K);
    %f=f/A;
    %h = figure(i);
    %movegui('east')
    %scatter3(X(1,:),X(2,:),f,[],f,'o','filled','MarkerEdgeColor','none')
    %colormap(jet)
    %axis([-4,4,-4,4]) %suport set ?(x)
    %view([0 90]) %Camera position
    %title(['Approximated PDF w.r.t iteration (',num2str(iter),')'])
    %saveas(h,sprintf('memk1-%d.png',iter));
